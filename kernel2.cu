...
__device__ void parentTrapPartduex(int scaler, int noEdges, int *edgesPN,
  ...
  for (int i = start; i < stop; i++) {
    for (int j = start2; j < start2 + noNodes; j++) {
      if (temp2[i] >= placeHolder[j] + edgesPN[blockIdx.x] &&
          temp2[i] < placeHolder[j + 1] + edgesPN[blockIdx.x]) {
...
__global__ void run3(int scaler, int *dedgesPN, int noEdges, int noNodes,
  ...
  parentTrap(scaler, blockIdx.x, index, noEdges, dedgesPN, out2, edgeAry,
             temp2);
  ...
  parentTrapPartduex(scaler, noEdges, dedgesPN, index, noNodes, placeHolder,
                     temp2, out2, out3);
...
__global__ void edgePerNetworkKernel(int *input, int *output, int *srchAry,
                                     int numNodes, const int maxParents,
                                     int c) {

  // edgePerNet(input, output, srchAry, numNodes, maxParents, c);

  // compute sum of edges for a single network and store in output[] - prefix
  // sum is completed on CPU

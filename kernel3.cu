...
__global__ void run22(int c, int *edgesPN, int *data, int *nodeAry, int noGenes,
  ...
  // nodeAry[index] = nodeMaker(c, index, sharedData, srchAry); //-shared memory
  // code
  ...
  if (threadIdx.x > 0) {
    edgeCounter(c, edgesPN, index, data, nodeAry, srchAry, edgeAry, maxParents);
  } // normal code
  // if (threadIdx.x > 0){ edgeCounter(c, edgesPN, index, data, nodeAry,
  // srchAry, edgeAry); } if (threadIdx.x > 0){ edgeCounter(c, edgesPN, index,
  // sharedData, nodeAry, srchAry, edgeAry, maxParents); } //-shared memory code
